...
<a href="<?php echo e(route('tpq.santri.index')); ?>" class="active">
...
<?php endif; ?>
    ...
    <main class="main">
        ...
        <div class="grid">
            ...
            <div class="section" id="form-container">
                ...
                <form id="santri-form" action="<?php echo e(route('tpq.santri.store')); ?>" method="POST">
                    <?php echo csrf_field(); ?>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="nis" name="nis" value="<?php echo e($nextNis); ?>" readonly placeholder="Masukkan NIS santri">
                    </div>
            ...
            <div class="section">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; flex-wrap: wrap; gap: 12px; border-bottom: 1px solid #e6f4ed; padding-bottom: 12px;">
                    <div style="display: flex; align-items: center; gap: 12px;">
                        ...
                        <a href="<?php echo e(route('tpq.santri.export.excel')); ?>" style="background: #10b981; color: white; padding: 6px 12px; border-radius: 8px; text-decoration: none; font-size: 12px; font-weight: 600;"><i class="fa-solid fa-file-excel"></i> Export Excel</a>
                        <a href="<?php echo e(route('tpq.santri.export.pdf')); ?>" target="_blank" style="background: #dc2626; color: white; padding: 6px 12px; border-radius: 8px; text-decoration: none; font-size: 12px; font-weight: 600;"><i class="fa-solid fa-file-pdf"></i> Export PDF</a>
                    </div>
                    <form action="<?php echo e(route('tpq.santri.index')); ?>" method="GET" style="display: flex; gap: 8px; align-items: center; margin: 0; flex-wrap: wrap;">
                        <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Cari nama santri..." style="padding: 8px 12px; border-radius: 8px; border: 1px solid #d1e7dd; font-size: 13px; width: 200px;">
                        ...
                        <select name="kelas_id" id="filter_kelas_id" onchange="this.form.submit()" style="padding: 8px 12px; border-radius: 8px; border: 1px solid #d1e7dd; font-size: 13px; background: #fbfffe; color: #103a2d; cursor: pointer; min-width: 160px; outline: none; transition: border-color 0.2s;">
                            ...
                            <option value="none" <?php echo e((isset($selectedKelasId) && $selectedKelasId === 'none') ? 'selected' : ''); ?>>Tanpa Kelas</option>
                            <?php $__currentLoopData = $kelas; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $k): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <option value="<?php echo e($k->id); ?>" <?php echo e((isset($selectedKelasId) && $selectedKelasId == $k->id) ? 'selected' : ''); ?>>
                        ...
                        <?php if(request('search') || request('kelas_id')): ?>
                            <a href="<?php echo e(route('tpq.santri.index')); ?>" style="padding: 8px 12px; border-radius: 8px; background: #ef4444; color: white; border: none; cursor: pointer; font-size: 13px; text-decoration: none; display: flex; align-items: center;">Reset</a>
                        <?php endif; ?>
                    </form>
                </div>
                    ...
                    <table>
                        ...
                        <tbody>
                            <?php $__empty_1 = true; $__currentLoopData = $santris; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $santri): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                <tr>
                                    <td><?php echo e($santris->firstItem() + $loop->index); ?></td>
                                    <td><?php echo e($santri->nis ?? '-'); ?></td>
                                    <td>
                                        <strong><?php echo e($santri->nama_santri); ?></strong>
                                        <?php if($santri->nama_panggilan): ?>
                                            <br><span style="font-size: 11px; color: #6b7280;">(<?php echo e($santri->nama_panggilan); ?>)</span>
                                        <?php endif; ?>
                                    </td>
                                    <td>
                                        <?php if($santri->kelas): ?>
                                            <span style="color:#0f766e; font-weight:600;">🏫 <?php echo e($santri->kelas->nama_kelas); ?></span>
                                        <?php else: ?>
                                            <span style="color:#9ca3af; font-style:italic;">Belum masuk kelas</span>
                                        <?php endif; ?>
                                    </td>
                                    <td><?php echo e($santri->jenis_kelamin); ?></td>
                                    <td><?php echo e($santri->tanggal_lahir ? date('d M Y', strtotime($santri->tanggal_lahir)) : '-'); ?></td>
                                    <td><?php echo e($santri->alamat_rumah ?? '-'); ?></td>
                                    <td><?php echo e($santri->nama_ayah ?? '-'); ?></td>
                                    <td><?php echo e($santri->nama_ibu ?? '-'); ?></td>
                                    <td><?php echo e($santri->no_hp_orang_tua ?? '-'); ?></td>
                                    <td>
                                        <button class="btn-action" onclick='editSantri(<?php echo json_encode($santri, 15, 512) ?>)'>Ubah</button>
                                        <form action="<?php echo e(route('tpq.santri.destroy', $santri->id)); ?>" method="POST" style="display:inline; margin-left: 4px;" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data santri ini?')">
                                            <?php echo csrf_field(); ?>
    ...
    <script>
        ...
        function editSantri(santri) {
            ...
            form.action = "<?php echo e(route('tpq.santri.update', ':id')); ?>".replace(':id', santri.id);
        ...
        function resetForm() {
            ...
            form.action = "<?php echo e(route('tpq.santri.store')); ?>";
...
<?php /**PATH C:\xampp\htdocs\masjid-baginda\resources\views/tpq/santri.blade.php ENDPATH**/ ?>