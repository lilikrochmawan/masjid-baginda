...
<body>
    ...
    <aside class="sidebar" id="sidebar">
        ...
        <nav class="sidebar-nav">
            <?php if(auth()->user()->hasAccess('keuangan.transaksi')): ?>
            <a href="<?php echo e(route('keuangan.index')); ?>">
                <span class="nav-icon">🏠</span> Beranda
            </a>
            <?php endif; ?>
            <?php if(auth()->user()->hasAccess('keuangan.laporan')): ?>
            <a href="<?php echo e(route('keuangan.laporan')); ?>">
                <span class="nav-icon">📊</span> Laporan
            </a>
            <?php endif; ?>
            <a href="<?php echo e(route('keuangan.event.index')); ?>" class="active">
                <span class="nav-icon">🎉</span> Keuangan Event
            </a>
            <div class="divider"></div>
            <?php if(auth()->user()->hasAccess('koin')): ?>
            <a href="<?php echo e(route('koin.index')); ?>">
    ...
    <main class="main">
        <div class="kop-header">
            ...
            <div class="kop-logo">
                <?php if($setting && $setting->logo): ?>
                    <img src="<?php echo e(asset('storage/' . $setting->logo)); ?>" alt="Logo Masjid" class="kop-logo-img">
                <?php else: ?>
                    <img src="<?php echo e(asset('images/image.png')); ?>" alt="Logo Masjid" class="kop-logo-img">
                <?php endif; ?>
            </div>
            <div class="kop-text">
        ...
        <div class="page-title" style="display: flex; justify-content: space-between; align-items: center;">
            <h1>Keuangan Event: <?php echo e($event->nama_event); ?></h1>
            <div>
                <button type="button" class="button-primary" style="background: #0f766e; margin-right: 8px;" onclick="window.print()">🖨️ Cetak</button>
                <a href="<?php echo e(route('keuangan.event.index')); ?>" class="button-primary" style="background: #64748b; text-decoration: none;">&larr; Kembali</a>
            </div>
        </div>
        ...
        <div class="grid">
            <div class="print-hide">
                <div class="section">
                    ...
                    <?php else: ?>
                        <form action="<?php echo e(route('keuangan.event.transaksi.store', $event->id)); ?>" method="POST">
                            <?php echo csrf_field(); ?>
                            <div class="form-group">
                                <label for="tanggal_kas">Tanggal</label>
                                <input type="date" id="tanggal_kas" name="tanggal_kas" value="<?php echo e(old('tanggal_kas', date('Y-m-d'))); ?>" required>
                            </div>
                            <div class="form-group">
                                <label for="tipe">Tipe Arus Kas</label>
                                <select id="tipe" name="tipe" required>
                                    <option value="">Pilih tipe</option>
                                    <option value="masuk" <?php echo e(old('tipe') === 'masuk' ? 'selected' : ''); ?>>Masuk</option>
                                    <option value="keluar" <?php echo e(old('tipe') === 'keluar' ? 'selected' : ''); ?>>Keluar</option>
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="jumlah">Jumlah</label>
                                <input type="number" id="jumlah" name="jumlah" value="<?php echo e(old('jumlah')); ?>" min="1" required>
                            </div>
                            <div class="form-group">
                                <label for="keterangan">Keterangan</label>
                                <textarea id="keterangan" name="keterangan"><?php echo e(old('keterangan')); ?></textarea>
                            </div>
                ...
                <div class="section">
                    ...
                    <div class="summary-grid">
                        <div class="summary-card">
                            ...
                            <strong><?php echo e(number_format($totalMasuk, 0, ',', '.')); ?></strong>
                        </div>
                        <div class="summary-card">
                            <span>Total Pengeluaran</span>
                            <strong><?php echo e(number_format($totalKeluar, 0, ',', '.')); ?></strong>
                        </div>
                        <div class="summary-card" style="<?php echo e($saldo < 0 ? 'background: #fee2e2;' : ''); ?>">
                            <span>Sisa Saldo</span>
                            <strong style="<?php echo e($saldo < 0 ? 'color: #991b1b;' : ''); ?>"><?php echo e(number_format($saldo, 0, ',', '.')); ?></strong>
                        </div>
                    </div>
                    ...
                    <div class="print-hide" style="margin-top: 20px; padding-top: 20px; border-top: 1px solid #e5e7eb;">
                        ...
                        <?php else: ?>
                            <form action="<?php echo e(route('keuangan.event.transfer', $event->id)); ?>" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin mentransfer sisa saldo Rp <?php echo e(number_format($saldo, 0, ',', '.')); ?> ke kas utama masjid? Setelah ditransfer, data event tidak bisa ditambah/diubah lagi.');">
                                <?php echo csrf_field(); ?>
                                <button type="submit" class="button-primary" style="width: 100%; <?php echo e($saldo <= 0 ? 'opacity: 0.5; pointer-events: none;' : ''); ?>">Transfer Saldo (Rp <?php echo e(number_format($saldo, 0, ',', '.')); ?>)</button>
                            </form>
                        <?php endif; ?>
                    </div>
                </div>
        ...
        <div class="section">
            ...
            <div class="table-wrapper">
                <table>
                    ...
                    <tbody>
                        <?php $__empty_1 = true; $__currentLoopData = $event->transaksis; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $trx): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <tr>
                                <td><?php echo e($loop->iteration); ?></td>
                                <td><?php echo e($trx->tanggal_kas); ?></td>
                                <td>
                                ...
                                <td><?php echo e(number_format($trx->jumlah, 0, ',', '.')); ?></td>
...
<?php /**PATH C:\xampp\htdocs\masjid-baginda\resources\views/keuangan/event/show.blade.php ENDPATH**/ ?>